calcPCClocks1 <- function(DNAm, pheno, RData = NULL) {
  # Input validation
  # Check DNAm
  check_DNAm(DNAm)
  # Check RData
  ...
  pheno$PCGrimAge <- as.numeric(as.matrix(pheno[, RData$CalcPCGrimAge$components]) %*% RData$CalcPCGrimAge$PCGrimAge.model + RData$CalcPCGrimAge$PCGrimAge.intercept)
