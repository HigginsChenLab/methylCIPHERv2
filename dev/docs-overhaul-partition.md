...
3. **Where principle 2 would drag a whole area into an unrelated file for one rule, the rule
   moves** to the area that already owns that file, or to the core if it is cross-cutting.
...
| coverage | `R/coverage.R`, `R/coverage_report.R`, `R/gap_reasons.R`, `R/constants.R` | 129, 134, 142, 147, 201, 237, 250, 258, 303, 308, 544 (the no-record half), 599, 609 | 5.4k |
...
**Core only, matched by no rules file:** `R/utils.R`, `R/mc-params.R`,
`R/methylCIPHERv2-package.R`, `R/sim_DNAm.R`. The list is written into the core, and the stage 5
check reads it: a file must match a glob or be on the list.
...
| File | Areas | Why |
|---|---|---|
| `R/score_cohort.R` | front-door, engine | The hub. `mc_cohort()` runs the gates in the order rule 573 depends on, and `score_cohort()` is the engine. |
...
| `R/coverage_report.R` | record, coverage | Holds both coverage frames and a `finalized()` call site. Heaviest load in the tree, about 11k. |
