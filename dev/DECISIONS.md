...
`as.data.frame()`, `clocks_accel()`, `clocks_coverage()` and `samples_coverage()` all drop it
through one helper, `drop_single_batch()` (`R/mc_result.R`). Nothing internal changed: `$provenance`
still always carries the per-sample vector, `clocks_accel()` still always offers `mc_batch_id` to
...
Third pass over the finalizers, driven by walking the rbind -> `clocks_accel` workflow. Always-on
...
is the one that forced it. First, it was about to become a formula variable in `clocks_accel`, and
...
**Every finalizer re-finalizes; `rbind` still does not.** `clocks_accel` was residualizing
...
## 2026-07-31 -- `clocks_accel` internals: one QR per row set, and the join gates what a join can break

Second pass over `R/clocks_accel.R`. Always-on suite 1020 pass / 0 fail / 260 skip. Parity not run;
`R CMD check` not run.
...
**The grouped fit stopped being an optimization.** `dev/clocks_accel.md` had deferred "group clocks
...
## 2026-07-31 -- building the finalizers: `clocks_accel`, no `collapse`, and what the record's pheno cannot answer

Built the surface designed in the entry below (`R/clocks_accel.R`,
...
**The verb is `clocks_accel()`, not `clock_accel()`.** The finalizer family is already
...
`clocks_accel()`. Built that, then removed it: the two consequences are the same consequence
...
in `clocks_accel()`, because `check_pheno()`'s hint ("add it to `pheno`") would send the caller
...
**So `clocks_accel(res)` failing on a covariate-free record is the design, not a papercut.** The
